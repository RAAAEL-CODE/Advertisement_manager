...
def show_add_event_page():
    ...
    # The rest of your content goes here, automatically placed next to the sidebar.
    # No need for the extra 'ui.row' or nested 'ui.column' for the sidebar.
    with ui.column().classes(
        ...
        categories = ui.select(
            ['Clothing', 'Tumblers', 'Cars', 'Furniture', 'Bags']
        ).props('outlined').classes('w-full')

        ui.label('Upload image').classes('font-medium text-gray-700')
        flyer = ui.upload(on_upload=handle_flyer_upload).classes(
            'w-full rounded-lg border border-dashed border-gray-400 p-4').props("color=black")
        ...
        def submit():
            ...
            response = add_event(ad_data, files={'flyer': flyer_content})
